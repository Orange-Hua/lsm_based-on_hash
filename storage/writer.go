...
func (w *Writer) Set(key, value []byte) error {
	...
	if w.cmp.Compare(w.prevKey, key) >= 0 {
		...
		log.Fatalf("prevKey：%d比key:%d大", internal.ByteArrayToInternalKey(w.prevKey).Key, internal.ByteArrayToInternalKey(key).Key)
		return w.err
	}
	...
	//c := 4 * (len(w.restarts) + 1)

	if len(w.buf)+4*(len(w.restarts)+1) >= w.blockSize {
		//log.Printf("storage/writer.go/Set:buf长度%d,重启点长度%d,块大小:%d,需要提交块", len(w.buf), c, w.blockSize)
		bh, err := w.finishBlock()
		if err != nil {
			w.err = err
			return w.err
		}
		//log.Println("storage/writer.go/Set():写完一个块")
		w.pendingBH = bh
	}
	return nil
}
...
func (w *Writer) append(key, value []byte, restart bool) {
	...
	if restart {
		...
		nShared = internal.SharedPrefixLen(w.prevKey, key)
	}
	...
	//l := len(key) + len(value)
	//log.Printf("storage/writer.go/append:  添加了一个key,value对共%d字节", l)
}
...
func (w *Writer) finishBlock() (blockHandle, error) {
	...
	if w.compression == internal.SnappyCompression {
...
func (w *Writer) Close() (err error) {
	...
	if w.nEntries > 0 || len(w.indexEntries) == 0 {
		...
		w.pendingBH = bh
		w.flushPendingBH(nil)
		//log.Println("storage/writer.go/Close: 文件关闭，完成写入文件最后一个块")
	}
...
// close the file.
func NewWriter(f internal.File, o *internal.Options) *Writer {
	w := &Writer{
		closer:               f,
